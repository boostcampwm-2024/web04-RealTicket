...
describe('Phase 2: 섹션 전환 엔드포인트 + 재연결 복원', () => {
  ...
  // TODO(02-04-PLAN): D-07 PATCH 제거 후 GET-based 시나리오로 정식 갱신 예정. 본 commit은 임시 skip.
  // ─── API-01, SSE-03, SSE-05: PATCH 섹션 전환 ───

  it.skip('API-01/SSE-05: PATCH /booking/seat/section → 200 + sectionIndex 포함', async () => {
  ...
  it.skip('API-01 (세션 갱신): PATCH 성공 후 subscribedSection이 요청한 섹션 번호로 갱신된다', async () => {
  ...
  it.skip('SSE-03 (idempotent): 같은 sectionIndex로 PATCH 두 번 → 두 번 모두 200', async () => {
  ...
  it.skip('API-02: 미인증 요청(쿠키 없음) → 403 FORBIDDEN', async () => {
  ...
  it.skip('VAL-01: 구독 섹션(0)과 다른 sectionIndex(1)로 bookSeat → 403 BOOKING_SEAT_UNAUTHORIZED_SECTION', async () => {
    await switchSection(app, userSid, 0).expect(200);
  ...
  it.skip('VAL-02: 구독 섹션(0)과 다른 sectionIndex(1)로 unBookSeat → 403 BOOKING_SEAT_UNAUTHORIZED_SECTION', async () => {
    await switchSection(app, userSid, 0).expect(200);
  ...
  it.skip('SSE-06: SSE 연결 해제 후 subscribedSection이 보존된다', async () => {
    await switchSection(app, userSid, 0).expect(200);
  ...
  it.skip('SSE-07: 재연결 후 subscribedSection이 이전 섹션으로 복원된다', async () => {
    await switchSection(app, userSid, 0).expect(200);
